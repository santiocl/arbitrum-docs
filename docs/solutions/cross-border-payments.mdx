...
flowchart TB
    subgraph Stack["Your application and service providers"]
    ...
    subgraph ArbitrumOne["Arbitrum One — stablecoin settlement"]
        ...
        STABLE[USD stables · EURe · MXNB · MYRC]
    end
...
Not every box is required for a pilot — most teams start with **custody + native USDC + one ramp service provider**. See the [four-stage flow](#how-cross-border-payments-work-on-arbitrum) above for Fund → Deliver.
...
- Your compliance and UX logic lives offchain or with compliance service providers (for example Chainalysis in the [payments ecosystem](#payments-ecosystem))
...
Most cross-border pilots use **existing** stablecoins (USDC, USDT, PYUSD, or non-USD-denominated assets such as EURe, MXNB, and MYRC). Issuing a custom ERC-20 is uncommon for standard payout apps; it applies mainly when you need **tokenized deposits**, **bank-issued settlement assets**, or a **custom gas token** on your own chain — not typical Mode 1 or Mode 2 paths.
