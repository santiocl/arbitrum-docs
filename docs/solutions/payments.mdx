...
Payments on Arbitrum reach **soft confirmation in sub-second** (sequencer inclusion) and **hard finality in minutes** once posted to Ethereum. See the [Transaction lifecycle](/how-arbitrum-works/deep-dives/transaction-lifecycle) and [security and finality for enterprise chains](https://blog.arbitrum.io/l1-vs-l2-choosing-the-right-chain-architecture-for-your-enterprise/#security-finality).

:::info[Start on Arbitrum One]
For most payment pilots, we recommend starting on Arbitrum One before building a [dedicated Arbitrum chain](#mode-3-launch-a-dedicated-arbitrum-chain). You get immediate access to billions in stablecoin liquidity with reduced infrastructure overhead.
:::
...
Three paths cover most payment builds: **Mode 1** settles on Arbitrum One without custom contracts; **Mode 2** adds payment smart contracts on Arbitrum One; **Mode 3** launches a dedicated Arbitrum chain when you need chain-level economics, compliance, or privacy. Details for each mode are below — use the [decision tree](#decision-tree) at the end of this section to choose.
...
Confidential amounts and counterparties can be implemented at the **smart contract** layer on Arbitrum One or at **chain** level on a dedicated Arbitrum chain. On Arbitrum One, explore [Fhenix CoFHE](https://cofhe-docs.fhenix.zone/) (encrypted contract logic on Arbitrum Sepolia today) and [Railgun](https://railgun.org/) (private transfers). Chain-level private execution is on the [Mode 3](#mode-3-launch-a-dedicated-arbitrum-chain) roadmap. For architecture scoping, [contact Offchain Enterprise Services](https://www.offchain.io/contact).
...
### Mode 3: Launch a dedicated Arbitrum chain {#mode-3-launch-a-dedicated-arbitrum-chain}
...
flowchart TD
    START([Payments use case]) --> Q1{Need chain-level control over<br/>network fees, compliance engine,<br/>or private execution?}
    ...
    M1 --> M1D[Stablecoin transfers via RPC or providers]
...
Most teams start in **Mode 1** and add Mode 2 or Mode 3 only when requirements demand it. The modes are **not mutually exclusive in capability**: Mode 3 is a **superset** — you can still deploy payment contracts on your chain, similar to Mode 2, while also controlling network fees, compliance, and data visibility.
...
A minimal path from zero to a verified settlement on Arbitrum One. Skip steps that do not apply to your mode (see the [decision tree](#decision-tree)).
